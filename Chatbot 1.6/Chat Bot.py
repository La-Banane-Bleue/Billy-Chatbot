...
def respond(question, responses):
    ...
    # Supprimer la ponctuation de la question, sauf "-"
    translation_table = str.maketrans("", "", string.punctuation.replace("-", ""))
    question = question.translate(translation_table)

    # Gérer le point d'interrogation
    if question[-1] == "?":
        question = question[:-1]

    # Chercher une correspondance partielle dans les clés des réponses
    matching_keys = [key for key in responses.keys() if key.replace("'", "") in question]

    if matching_keys:
        # Retourner la première réponse correspondante
...
def submit_question(event=None):
    ...
    # Vérifie si l'utilisateur a trouvé l'easter egg
