...
def load_settings(file):
    settings = {'Theme': 'Light', 'ChatbotName': 'Billy'}  # Ajout de la clé 'ChatbotName'
    if os.path.exists(file):
        with open(file, "r") as f:
            for line in f:
                key, value = line.strip().split("=")
...
def submit_question(event=None):
    ...
    chat_responses.append((f"Vous: {user_input}", f"{settings['ChatbotName']}: {response}"))
...
def propose_response():
    ...
    save_button = tk.Button(propose_window, text="Enregistrer", font=("Arial", 12), relief=tk.RAISED, command=save_proposed_response)
    save_button.pack()
...
def change_chatbot_name():
    # Ouvre une fenêtre pour changer le nom du chatbot
    name_window = tk.Toplevel(window)
    name_window.title("Changer le nom du Chatbot")

    name_label = tk.Label(name_window, text="Nouveau nom du Chatbot :")
    name_label.pack()

    name_entry = tk.Entry(name_window, font=("Arial", 12), bd=1, relief=tk.SOLID)
    name_entry.pack()

    def save_new_name():
        new_name = name_entry.get()
        settings["ChatbotName"] = new_name
        save_settings(settings_file, settings)
        name_window.destroy()

    save_button = tk.Button(name_window, text="Enregistrer", font=("Arial", 12), relief=tk.RAISED, command=save_new_name)
    save_button.pack()
...
def show_settings():
    ...
    change_name_button = tk.Button(settings_window, text="Changer le nom du Chatbot", font=("Arial", 12), relief=tk.RAISED, command=change_chatbot_name)
    change_name_button.pack()

    version_label = tk.Label(settings_window, text="Version : 1.9")
    version_label.pack()
